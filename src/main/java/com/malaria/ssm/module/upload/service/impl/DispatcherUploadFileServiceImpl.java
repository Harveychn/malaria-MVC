...
import com.malaria.ssm.module.upload.service.DispatcherUploadFileService;
    ...
    private int getCaseNum(List<String> fieldNames) throws Exception {
        if (45 == fieldNames.size()) {
            if (fieldNames.get(0).trim().equals("数据年份")
                    ...
                    && fieldNames.get(5).trim().equals("患儿家长姓名")
                    && fieldNames.get(6).trim().equals("身份证号")
                    && fieldNames.get(7).trim().equals("性别")
                    && fieldNames.get(8).trim().equals("出生日期")
                    && fieldNames.get(9).trim().equals("年龄")
                    && fieldNames.get(10).trim().equals("患者工作单位")
                    && fieldNames.get(11).trim().equals("联系电话")
                    && fieldNames.get(12).trim().equals("病人属于")
                    && fieldNames.get(13).trim().equals("现住地址国标")
                    && fieldNames.get(14).trim().equals("现住详细地址")
                    && fieldNames.get(15).trim().equals("职业")
                    && fieldNames.get(16).trim().equals("病例分类")
                    && fieldNames.get(17).trim().equals("病例分类2")
                    && fieldNames.get(18).trim().equals("发病日期")
                    && fieldNames.get(19).trim().equals("诊断时间")
                    && fieldNames.get(20).trim().equals("死亡日期")
                    && fieldNames.get(21).trim().equals("疾病名称")
                    && fieldNames.get(22).trim().equals("订正前病种")
                    && fieldNames.get(23).trim().equals("填卡医生")
                    && fieldNames.get(24).trim().equals("医生填卡日期")
                    && fieldNames.get(25).trim().equals("报告单位地区编码")
                    && fieldNames.get(26).trim().equals("报告单位")
                    && fieldNames.get(27).trim().equals("单位类型")
                    && fieldNames.get(28).trim().equals("报告卡录入时间")
                    && fieldNames.get(29).trim().equals("录卡用户")
                    && fieldNames.get(30).trim().equals("录卡用户所属单位")
                    && fieldNames.get(31).trim().equals("县区审核时间")
                    && fieldNames.get(32).trim().equals("地市审核时间")
                    && fieldNames.get(33).trim().equals("省市审核时间")
                    && fieldNames.get(34).trim().equals("审核状态")
                    && fieldNames.get(35).trim().equals("订正报告时间")
                    && fieldNames.get(36).trim().equals("订正终审时间")
                    && fieldNames.get(37).trim().equals("终审死亡时间")
                    && fieldNames.get(38).trim().equals("订正用户")
                    && fieldNames.get(39).trim().equals("订正用户所属单位")
                    && fieldNames.get(40).trim().equals("删除时间")
                    && fieldNames.get(41).trim().equals("删除用户")
                    && fieldNames.get(42).trim().equals("删除用户所属单位")
                    && fieldNames.get(43).trim().equals("删除原因")
                    && fieldNames.get(44).trim().equals("备注")
                    ) {
                return 1;
            }
        }
